...
if ! swapon --show 2>/dev/null | grep -v '^NAME ' | grep -q '^'; then
  ...
  mkdir -p "/var/cache/swaps/swapFile"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
SCRIPT_OS="AlmaLinux"
SCRIPT_DESCRIBE="Minimal OracleCloud"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
SERVICES_ENABLE="cockpit cockpit.socket httpd munin-node nginx ntpd php-fpm postfix proftpd rsyslog snmpd sshd uptimed"
